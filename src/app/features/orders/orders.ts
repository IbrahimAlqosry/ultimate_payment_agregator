...
/** Mock-only — `/payment-points`, the Platform-operator-wide list. Real `GET /payment-points`
 * (2026-09-16 OpenAPI update) confirmed live `403` for Platform sessions, so there is still no
 * real operator-wide view; see real-payment-points.ts for the Merchant/FI equivalents, which are
 * real. This screen used to also serve the Merchant/FI scopes via route-data `scope` — those
 * routes now point at the real component instead, so this component and template only ever run
 * in the operator/"all" layout. */
@Component({
  selector: 'app-orders',
...
export class Orders {
  ...
  load(silent = false): void {
    ...
    this.api.paymentPoints(this.query, 'all').subscribe({
      next: (rows) => {
        this.all.set(rows);
